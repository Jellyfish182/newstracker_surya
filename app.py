...
def cached_apollo_lookup(company, api_key):
    """One company's contacts, cached for a day.

    Apollo credits are finite, and the CFO of a company does not change
    between two scans an hour apart.
    """
    return analysis.enrich_with_apollo(company, api_key)
...
def run_scan(provider, api_key, model, effort, max_candidates,
             fresh_feeds=False, apollo_key="", apollo_limit=0):
    ...
    analysis.attach_contact_research(opportunities)

    if apollo_key and apollo_limit:
        # Highest-scoring leads first: credits are limited, so spend them on
        # the companies actually worth calling.
        ranked = sorted(opportunities, key=lambda o: o.get("score", 0), reverse=True)
        for position, opp in enumerate(ranked[:apollo_limit], start=1):
            progress.progress(
                min(0.99, 0.9 + (position / max(apollo_limit, 1)) * 0.1),
                text=f"Looking up contacts... {position}/{min(apollo_limit, len(ranked))}",
            )
            opp["apollo_contacts"] = cached_apollo_lookup(
                opp.get("company", ""), apollo_key)
...
def to_dataframe(rows):
    ...
    for row in rows:
        ...
        records.append({
            ...
            "Who To Approach": ", ".join(row.get("target_roles", [])),
            "People Named": ", ".join(row.get("people_named", [])),
            "Apollo Contacts": "; ".join(
                f"{c['name']} ({c['title']})" for c in row.get("apollo_contacts", [])),
            "LinkedIn Search": next(
                (l["url"] for l in row.get("contact_links", [])
                 if l["label"] == "LinkedIn people"), ""),
...
with st.sidebar:
    ...
    apollo_key = analysis.get_apollo_key()
    if apollo_key:
        st.markdown("### Contact lookup")
        st.markdown('<span class="pill pill-green">● Apollo connected</span>',
                    unsafe_allow_html=True)
        apollo_limit = st.slider(
            "Look up contacts for top N leads", 0, 25, 10, 1,
            help="Each company costs Apollo credits, so this is capped. "
                 "Results are cached for 24 hours.",
        )
    else:
        apollo_limit = 0
        with st.expander("Add verified contacts"):
            st.caption(
                "Add `APOLLO_API_KEY` to secrets to pull finance contacts "
                "(name, title, LinkedIn) for your top leads automatically."
            )
...
""",
        ...
        for day, items in by_day.items():
            ...
            for opp in sorted(items, key=lambda o: o["score"], reverse=True):
                ...
                contacts = ""
                links = opp.get("contact_links") or []
                if links:
                    roles = " · ".join(esc(r) for r in opp.get("target_roles", [])[:4])
                    link_html = " ".join(
                        f'<a class="research-link" href="{esc(l["url"])}" '
                        f'target="_blank" title="{esc(l["hint"])}">{esc(l["label"])}</a>'
                        for l in links
                    )
                    apollo_html = ""
                    if opp.get("apollo_contacts"):
                        rows = "".join(
                            '<div class="contact-row">'
                            f'<span class="contact-name">{esc(c["name"])}</span>'
                            f'<span class="contact-title">{esc(c["title"])}</span>'
                            + (f'<a class="contact-li" href="{esc(c["linkedin"])}" '
                               'target="_blank">LinkedIn</a>' if c.get("linkedin") else "")
                            + '</div>'
                            for c in opp["apollo_contacts"][:5]
                        )
                        apollo_html = (
                            '<div class="contact-list">'
                            '<div class="contact-head">Finance contacts</div>'
                            f'{rows}</div>'
                        )

                    named = ""
                    if opp.get("people_named"):
                        people = " · ".join(esc(p) for p in opp["people_named"][:4])
                        named = f'<div class="research-named">Named in article: {people}</div>'
                    contacts = (
                        '<div class="research">'
                        f'<div class="research-roles"><strong>Who to approach:</strong> {roles}</div>'
                        f'{apollo_html}'
                        f'{named}'
                        f'<div class="research-links">{link_html}</div>'
                        '</div>'
                    )
