...
def cached_groq_models(api_key):
    """Live model list from Groq, refreshed hourly.

    Groq retires and renames models, so a hardcoded list goes stale and
    every scan then fails with a 404 - the same trap Gemini set earlier.
    """
    return analysis.list_groq_models(api_key)
...
def run_scan(provider, api_key, model, effort, max_candidates, fresh_feeds=False):
    ...
    label = {"gemini": "Gemini", "claude": "Claude",
             "groq": "Groq"}.get(provider, "")
    ...
    if provider in ("gemini", "claude", "groq"):
    ...
    if provider == "groq":
        opportunities, rejected, telemetry = analysis.analyse_with_groq(
...
with st.sidebar:
    ...
    if len(engines) == 1:
        ...
        st.caption(
            "No API key found. Add `GROQ_API_KEY` or `GEMINI_API_KEY` for a "
            "free tier, or `ANTHROPIC_API_KEY` for Claude."
        )
    else:
    ...
    if engine == "Groq (free)":
        provider = "groq"
        st.markdown('<span class="pill pill-green">● Groq connected — free tier</span>',
                    unsafe_allow_html=True)
        groq_models = cached_groq_models(groq_key)
        default_groq = analysis.default_groq_model(groq_models)
        model = st.selectbox(
            "Model", options=groq_models,
            index=groq_models.index(default_groq) if default_groq in groq_models else 0,
            help="Fetched live from Groq, so retired names never appear.",
        )
        st.caption("Free tier, very fast. A separate daily quota from Gemini, "
                   "so a good fallback when Gemini runs out.")

    elif engine == "Gemini (free)":
        provider = "gemini"
...
engine_pill = {
    "groq": '<span class="pill pill-green">AI analysis - Groq</span>',
...
if scan_clicked:
    active_key = {"gemini": gemini_key, "groq": groq_key}.get(provider, claude_key)
