...
def run_scan(api_key, use_ai, model, effort, max_candidates):
    """Fetch -> prefilter -> analyse. Returns a result dict held in session."""
    progress = st.progress(0.0, text="Connecting to feeds...")

    def on_feed(done, total, name):
        progress.progress(done / total * 0.45, text=f"Reading feeds... {done}/{total}  ·  {name}")

    feed_results = feeds.fetch_all_feeds(progress_callback=on_feed)
    ...
    if not candidates:
        ...
        return {
            ...
            "engine": "AI" if use_ai else "Keyword",
    ...
    if use_ai:
        def on_batch(done, total):
            progress.progress(
                0.5 + (done / total) * 0.5,
                text=f"Claude is analysing articles... batch {done}/{total}",
            )
...
resolved_key = analysis.get_api_key()
ai_ready = analysis.ai_available(resolved_key)
...
with st.sidebar:
    ...
    if ai_ready:
        st.markdown(
            '<span class="pill pill-green">● Claude connected</span>',
            unsafe_allow_html=True,
        )
        use_ai = st.toggle("Use AI analysis", value=True,
                           help="Claude reads each article and judges relevance on meaning. "
                                "Turn off to use the offline keyword scorer.")
        effort = st.select_slider(
            "Analysis depth", options=["low", "medium", "high"], value="medium",
            help="Higher depth means more careful reading, slower scans and higher cost.",
        )
        model = "claude-opus-5"
    else:
        st.markdown(
            '<span class="pill pill-amber">● Keyword mode</span>',
            unsafe_allow_html=True,
        )
        st.caption(
            "No API key found. Add `ANTHROPIC_API_KEY` to `.streamlit/secrets.toml` "
            "or your environment to switch on AI analysis."
        )
        use_ai, effort, model = False, "medium", "keyword-fallback"

    st.markdown("### Scan settings")
    max_candidates = st.slider(
        "Max articles to analyse", 20, 200, analysis.MAX_CANDIDATES, 10,
        help="Caps cost and scan time. Newest articles are kept first.",
    )
    if use_ai:
    ...
    if st.session_state.get("scan"):
        scanned = st.session_state["scan"]["scanned_at"]
        st.caption(f"Last scan: {feeds.format_absolute(scanned)}")
...
engine_pill = (
    '<span class="pill pill-green">AI analysis</span>' if (ai_ready and use_ai)
    else '<span class="pill pill-amber">Keyword mode</span>'
)
...
if scan_clicked:
    st.session_state["scan"] = run_scan(resolved_key, use_ai, model, effort, max_candidates)
...
""",
    ...
    if telemetry.get("batches"):
        ...
        t4.metric("Est. cost", f"${telemetry['estimated_cost_usd']:.3f}")
