...
# --- Apollo.io (optional, licensed contact data) ----------------------
#
# Not wired into the UI yet. Drop an APOLLO_API_KEY into secrets and call
# enrich_with_apollo() from attach_contact_research() to populate verified
# names, titles and work emails. Apollo is used rather than scraping
# because it holds contact data under its own consent framework.

APOLLO_TITLES = [
    "CFO", "Chief Financial Officer", "Head of Tax", "Tax Director",
    "Finance Controller", "Company Secretary", "VP Finance",
]
...
def apollo_available():
    return bool(get_apollo_key())


def enrich_with_apollo(company, api_key, titles=None, limit=5):
    """Look up finance contacts at one company via Apollo's people search.

    Returns a list of {name, title, email, linkedin} dicts, or [] on any
    failure - enrichment must never break a scan.
    """
    import urllib.error
