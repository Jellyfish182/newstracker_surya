...
# --------------------------------------------------------------------
# Groq engine (free tier, OpenAI-compatible)
# --------------------------------------------------------------------
#
# Groq is a second free option and a useful fallback when Gemini's daily
# quota runs out. Its API is OpenAI-shaped: bearer auth, chat/completions,
# and response_format for structured output.
#
# Two differences from the OpenAI defaults are worth remembering:
#   - max_tokens is deprecated here; the parameter is max_completion_tokens.
#   - json_schema support varies by model, so a json_object fallback is
#     needed for models that reject a schema.

GROQ_URL = "https://api.groq.com/openai/v1/chat/completions"
GROQ_MODELS_URL = "https://api.groq.com/openai/v1/models"

# Only used if the live model list cannot be fetched. Groq renames and
# retires models, so the dropdown asks the API first.
...
# "Python-urllib/3.x" User-Agent with HTTP 403 and Cloudflare error 1010 -
# the request never reaches Groq at all. A normal browser User-Agent gets
# through. Without this every batch fails with a 403 that looks like an
# auth problem but is not one.
GROQ_HEADERS = {
...
# Not general chat models, however they are named.
_GROQ_EXCLUDE = ("whisper", "tts", "guard", "embed", "vision", "prompt-guard")


def get_groq_key():
    """Resolve the Groq key from Streamlit secrets first, then env."""
    try:
        import streamlit as st
...
def groq_available(api_key=None):
    key = api_key if api_key is not None else get_groq_key()
    return bool(key)


def list_groq_models(api_key):
    """Ask Groq which chat models this key can use, largest first."""
    ...
    try:
        request = urllib.request.Request(
            GROQ_MODELS_URL,
            headers=dict(GROQ_HEADERS, Authorization=f"Bearer {api_key}"),
        )
    ...
    names = [
        ...
        if m.get("id") and not any(x in m["id"].lower() for x in _GROQ_EXCLUDE)
    ]
    if not names:
        return GROQ_FALLBACK_MODELS

    # Bigger models first - they judge borderline stories better, and Groq is
    # fast enough that the size penalty barely shows.
    def rank(name):
        match = re.search(r"(\d+)\s*b\b", name.lower())
        return int(match.group(1)) if match else 0

    return sorted(set(names), key=lambda n: (-rank(n), n))


def _groq_size(name):
    """Parameter count in billions, read off the model name. 0 if absent."""
    match = re.search(r"(\d+)\s*b\b", (name or "").lower())
    return int(match.group(1)) if match else 0


def default_groq_model(available):
    """Largest model available - it judges borderline stories best, and Groq
    is fast enough that the size penalty barely shows.

    Sorts rather than trusting the caller's order.
    """
    if not available:
        return GROQ_FALLBACK_MODELS[0]
    return max(available, key=_groq_size)
...
def _groq_batch(batch, model, api_key):
    """One Groq call, retrying on rate limits."""
    import time as _time
    import urllib.error
    ...
    base = {
        ...
        "temperature": 0,
        # Not max_tokens - that name is deprecated on Groq.
        "max_completion_tokens": 8000,
    }

    schema_payload = dict(base, response_format={
        "type": "json_schema",
        "json_schema": {
            "name": "lead_screening",
    ...
    # Some models reject a schema outright; JSON mode works everywhere, with
    # the shape described in the prompt instead.
    json_payload = dict(base, response_format={"type": "json_object"})
    json_payload["messages"] = [
        {"role": "system", "content": SYSTEM_PROMPT + GEMINI_SYSTEM_SUFFIX},
        base["messages"][1],
    ]
    ...
    for attempt in range(GROQ_RETRIES):
        try:
            data = _groq_post(payload, api_key)
            ...
            if exc.code == 429:
                _time.sleep(GROQ_BACKOFF * (2 ** attempt))
                continue
            if exc.code == 400 and payload is schema_payload:
                # Model does not accept a JSON schema - drop to JSON mode.
                payload = json_payload
                continue
            raise RuntimeError(f"Groq request rejected: {last_error}")
        except (json.JSONDecodeError, KeyError, IndexError) as exc:
            last_error = f"{type(exc).__name__}: {exc}"
            _time.sleep(2)

    raise RuntimeError(f"Groq failed after {GROQ_RETRIES} attempts: {last_error}")


def analyse_with_groq(candidates, api_key, model=None, progress_callback=None):
    """Same contract as analyse_with_gemini, on Groq's free tier."""
    model = model or GROQ_FALLBACK_MODELS[0]
    batches = [candidates[i:i + BATCH_SIZE] for i in range(0, len(candidates), BATCH_SIZE)]
    leads, rejected, errors_seen = [], [], []
    totals = {"input_tokens": 0, "output_tokens": 0, "cache_read": 0}

    def run(index, batch):
        payload, usage = _groq_batch(batch, model, api_key)
        return index, batch, payload.get("results", []), usage
