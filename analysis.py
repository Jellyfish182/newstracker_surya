...
GEMINI_WORKERS = 4       # free tier allows ~10-15 req/min; 4 stays inside it
GEMINI_RETRIES = 4
GEMINI_BACKOFF = 3       # seconds, doubled each attempt: 3, 6, 12, 24
GEMINI_TIMEOUT_MS = 90_000   # a hung request must not stall the whole scan
        ...
        "results": {
            ...
            "items": {
                ...
                "required": [
                    ...
                    "tax_services", "pitch_note", "people_named", "evidence",
            ...
            try:
                ...
                for idx, article in enumerate(batch):
                    ...
                    record.update({
                        ...
                        "people_named": verdict.get("people_named") or [],
...
def _gemini_batch(client, batch, model):
    ...
    # Thinking is unnecessary for a rubric-driven classification and makes
    # every batch several times slower.
    #
    # The parameter differs by generation, and getting it wrong fails
    # SILENTLY: Gemini 3.x ignores thinking_budget entirely and falls back to
    # its default of medium thinking, so the request still succeeds - just
    # slowly. Gemini 3.x wants thinking_level; 2.x wants thinking_budget.
    ...
    for attempt in range(GEMINI_RETRIES):
        try:
            ...
            if "429" not in str(exc) and "RESOURCE_EXHAUSTED" not in str(exc).upper():
                raise
            last_error = exc
            _time.sleep(GEMINI_BACKOFF * (2 ** attempt))
...
def analyse_with_gemini(candidates, api_key, model=DEFAULT_GEMINI_MODEL,
    ...
    client = genai.Client(
        api_key=api_key,
        http_options=_types.HttpOptions(timeout=GEMINI_TIMEOUT_MS),
    )
    batches = [candidates[i:i + BATCH_SIZE] for i in range(0, len(candidates), BATCH_SIZE)]
...
# --------------------------------------------------------------------
#
# This layer deliberately does NOT scrape LinkedIn or harvest personal
# contact details. Scraping breaches LinkedIn's terms and risks the user's
# own account, and bulk collection of individuals' contact data engages the
# DPDP Act 2023. What it does instead is (a) say which job titles are worth
# approaching for this kind of event, (b) build ready-made search URLs the
# user clicks themselves, and (c) surface executives the article already
# names. For verified contact data there is an Apollo hook at the bottom -
# a licensed provider with a consent framework, which is the correct route.

# Who actually owns the decision, by event type. Ordered most useful first.
TARGET_ROLES = {
    "New GCC / Capability Centre": [
        "Head of Tax", "CFO", "Finance Controller", "Company Secretary",
    ],
    "Foreign Subsidiary / India Entry": [
        "Country Manager India", "CFO", "Head of Tax", "Company Secretary",
    ],
    "New Entity / Company Incorporation": [
        "Company Secretary", "CFO", "Finance Controller",
    ],
    "Office / Campus Setup": [
        "Finance Controller", "CFO", "Head of Real Estate",
    ],
    "Manufacturing Plant / Project": [
        "CFO", "Head of Indirect Tax", "Plant Finance Head", "Head of Projects",
    ],
    "Data Centre": [
        "CFO", "Head of Tax", "Head of Infrastructure",
    ],
    "R&D / Innovation Centre": [
        "Head of Tax", "CFO", "R&D Finance Lead",
    ],
    "Major Investment / Capex": [
        "CFO", "Head of Tax", "Treasurer", "Head of Strategy",
    ],
    "M&A / JV": [
        "Head of M&A", "CFO", "General Counsel", "Head of Tax",
    ],
    "Expansion of Existing Operations": [
        "Finance Controller", "Head of Tax", "CFO",
    ],
}

DEFAULT_ROLES = ["CFO", "Head of Tax", "Company Secretary"]

# Legal-suffix noise that hurts a people search more than it helps.
_SEARCH_STRIP = re.compile(
    r"\b(pvt|private|ltd|limited|inc|inc\.|llp|llc|plc|corp|corporation|"
    r"gmbh|india|technologies|solutions)\b\.?", re.IGNORECASE)


def _search_name(company):
    """Trim a company name down to what works in a people search.

    Keyword mode sometimes leaves a verb phrase attached ("Bharat Forge's
    Agneyastra Energetics Breaks Ground"), which poisons the search. Cut at
    the first verb, then drop legal suffixes.
    """
    if not company:
        return ""
    ...
    for token in company.split():
        if token.lower().strip(".,:;'") in _NAME_STOP_WORDS:
            break
        kept.append(token)
    trimmed = " ".join(kept) or company

    # Two or three words is the sweet spot for a people search; more just
    # narrows it to nothing.
    trimmed = " ".join(trimmed.split()[:4])

    cleaned = _SEARCH_STRIP.sub("", trimmed).strip(" ,.-'")
    cleaned = re.sub(r"\s{2,}", " ", cleaned)
    return cleaned or trimmed


def target_roles(event_type):
    return TARGET_ROLES.get(event_type, DEFAULT_ROLES)


def build_contact_links(company, event_type, location=""):
    """Ready-made research URLs. Nothing is fetched or stored - these are
    links the user clicks, so no scraping and no personal data held here."""
    from urllib.parse import quote_plus

    if not company or company == "Not named":
        return []

    name = _search_name(company)
    roles = target_roles(event_type)
    role_query = " OR ".join(f'"{r}"' for r in roles[:3])

    return [
        {
            "label": "LinkedIn people",
            "hint": f"{roles[0]} and similar at {name}",
            "url": "https://www.linkedin.com/search/results/people/?keywords="
                   + quote_plus(f"{name} {roles[0]}"),
        },
        {
            "label": "Google",
            "hint": "Finance leadership, India",
            "url": "https://www.google.com/search?q="
                   + quote_plus(f'"{name}" India ({role_query})'),
        },
        {
            "label": "Zauba Corp",
            "hint": "Directors on the MCA register",
            "url": "https://www.zaubacorp.com/companysearch/" + quote_plus(name),
        },
        {
            "label": "Tofler",
            "hint": "Filings, signatories, financials",
            "url": "https://www.tofler.in/search?query=" + quote_plus(name),
        },
        {
            "label": "News",
            "hint": "Recent coverage to reference on the call",
            "url": "https://news.google.com/search?q="
                   + quote_plus(f"{name} {location}".strip())
                   + "&hl=en-IN&gl=IN&ceid=IN:en",
        },
    ]


def attach_contact_research(leads):
    """Add roles and search links to each lead, in place."""
    for lead in leads:
        company = lead.get("company", "")
        event = lead.get("event_type", "")
        lead["target_roles"] = target_roles(event)
        lead["contact_links"] = build_contact_links(
            company, event, lead.get("location", ""))
    return leads


# --- Apollo.io (licensed contact data) --------------------------------
#
# Apollo is used rather than scraping because it licenses contact data under
# its own consent framework. Two calls are needed per company:
#
#   1. Organization Search  -> resolve a company NAME to an Apollo org id.
#      The people endpoint filters on org id or domain, never on a name.
#   2. People Search        -> finance contacts at that org.
#
# The people endpoint deliberately does NOT return email addresses; Apollo
# gates those behind separate enrichment calls that burn email credits. What
# comes back is name, title and LinkedIn URL, which is enough to reach the
# right person, and costs far less of a limited free-tier allowance.
#
# Note the endpoint path: /mixed_people/api_search. The older
# /mixed_people/search returns 403 on Basic and free plans.
...
APOLLO_TITLES = [
    ...
    "Head of Tax", "Tax Director", "Head of Taxation", "Finance Controller",
    "Financial Controller", "Company Secretary", "Head of Finance",
]
...
def get_apollo_key():
    """Resolve the Apollo key from Streamlit secrets first, then env."""
    try:
        import streamlit as st
...
def apollo_available(api_key=None):
    key = api_key if api_key is not None else get_apollo_key()
    return bool(key)


def _apollo_post(url, params, api_key):
    """POST to Apollo. Parameters go in the query string, per their docs.

    Returns the decoded JSON, or None on any failure - contact enrichment is
    a bonus and must never break a scan.
    """
    import urllib.parse
    ...
    try:
        with urllib.request.urlopen(request, timeout=APOLLO_TIMEOUT) as response:
            return json.loads(response.read())
    except Exception:
        return None


def apollo_find_organization(company, api_key):
    """Company name -> {id, name, domain}. None when nothing matches."""
    if not api_key or not company:
        return None

    data = _apollo_post(APOLLO_ORG_URL, {
        "q_organization_name": _search_name(company),
        "page": 1,
        "per_page": 1,
    }, api_key)
    if not data:
...
def enrich_with_apollo(company, api_key, titles=None, limit=5, india_only=True):
    """Finance contacts at one company.

    Returns a list of {name, title, linkedin, has_email} dicts. Empty list on
    any failure. Emails are not included - see the note above.
    """
    org = apollo_find_organization(company, api_key)
    if not org:
        return []

    params = {
        "person_titles[]": titles or APOLLO_TITLES,
        "page": 1,
    ...
    if org.get("id"):
        ...
        params["q_organization_domains_list[]"] = [org["domain"]]
    else:
        return []
    if india_only:
        params["person_locations[]"] = ["India"]

    data = _apollo_post(APOLLO_PEOPLE_URL, params, api_key)
    if not data:
        return []
